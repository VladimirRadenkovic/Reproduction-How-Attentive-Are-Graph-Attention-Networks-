...
class Experiment():
    def __init__(self, args):
        ...
        np.random.seed(seed)
        random.seed(seed)
        # For each permutation of keys, a graph is created with the corresponding values as node features.
        # The number of graphs in the dataset is equal to the number of permutations of the keys.
    ...
    def print_args(self, args):
        ...
        print()
    # run is a method that trains a graph neural network on a given task using the specified hyperparameters.
    def run(self):
        ...
        for epoch in range(1, (self.max_epochs // self.eval_every) + 1):
            self.model.train()
            # DataLoader returns mini-batches of training data of batch_size
    ...
    def run1(self):
        ...
        for epoch in range(1, self.max_epochs + 1):
            self.model.train()
            # DataLoader returns mini-batches of training data of batch_size
            loader = DataLoader(self.X_train*10, batch_size=self.batch_size, shuffle=True,
                                pin_memory=True, num_workers=self.loader_workers)
            ...
            scheduler.step(relevant_value)
            print(
                f'Epoch {epoch}, LR: {cur_lr}: Train loss: {avg_training_loss:.7f}, '
                f'Train-node acc: {train_per_node_acc:.4f}, '
                f'Train-graph acc: {train_per_graph_acc:.4f}, '
                f'Test-node acc: {test_node_acc:.4f}, '
                f'Test-graph acc: {test_graph_acc:.4f} {new_best_str}')
